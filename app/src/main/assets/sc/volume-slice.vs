...
// Render uniforms
uniform mediump mat4 systemScaleM;
uniform mat4 M;
uniform mat4 V;
uniform mat4 P;
uniform mat4 S;
...
void main (void) {
	for (int i=0; i<4; i++) {
		...
		float lambda = denominator != 0.0 ? ((dPlane+dPlaneStep*float(gl_InstanceID))-dot(np, vertex1))/denominator : -1.0;
		...
		if (lambda >= 0.0 && lambda <= 1.0) {
			...
			gl_Position = P * V * systemScaleM * (vertexStart + lambda*e);
			return;
		}
	}
}